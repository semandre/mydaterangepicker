...
gulp.task('tsc.compile.bundle', function (cb) {
    exec(tsc + ' -p ./tsconfig.bundle.json && ' + uglify + ' dist/mydaterangepicker.bundle.js --screw-ie8 --compress --mangle --output dist/mydaterangepicker.bundle.min.js', function (err, stdout, stderr) {
        console.log(stdout);
        console.log(stderr);
        cb(err);
    });
...
gulp.task('copy.styles.template.dist', function() {
    return gulp.src('./src/my-date-range-picker/my-date-range-picker.component.{html,css}').pipe(gulp.dest('./dist'));
});
